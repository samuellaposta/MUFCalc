...
static QLineEdit* makeValidatedField(const QString& ph, int maxLen) {
    ...
    le->setStyleSheet(
        ...
        "color:#111827;padding:9px 12px;font-size:14px;}"
...
void DataEntryWizard::buildUI() {
    ...
    t1->setStyleSheet("color:white;font-size:16px;font-weight:800;");
    auto* t2 = new QLabel("Material Balance Period Data Entry");
    t2->setStyleSheet("color:#a5c8e8;font-size:13px;letter-spacing:1px;");
    ...
    secBadge->setStyleSheet("color:#a5c8e8;font-size:13px;letter-spacing:1px;");
    ...
    m_pageIndicator->setStyleSheet("color:#111827;font-size:14px;font-family:'Courier New',monospace;");
    ...
    btnLoad->setStyleSheet("QPushButton{background:#f0f7e8;color:#3d7520;border:1.5px solid #5da832;"
                           "border-radius:7px;padding:10px 18px;font-weight:600;font-size:13px;}"
    ...
    m_btnCalc->setStyleSheet(Styles::greenButtonStyle() + "QPushButton{padding:11px 30px;font-size:14px;}");
    m_btnCalc->hide();
...
QWidget* DataEntryWizard::buildPage1() {
    ...
    auto mkLbl = [](const QString& txt, const QString& hint = "") {
        ...
        l->setStyleSheet("color:#374151;font-weight:700;font-size:14px;");
        col->addWidget(l);
        if (!hint.isEmpty()) {
            auto* h = new QLabel(hint);
            h->setStyleSheet("color:#111827;font-size:13px;");
            col->addWidget(h);
        }
    ...
    auto* snIcon = new QLabel("ℹ️"); snIcon->setStyleSheet("font-size:16px;");
    auto* snTxt = new QLabel("All fields are validated against injection attacks. Characters < > ; \" ' \\ | ` $ { } are blocked.");
    snTxt->setStyleSheet("color:#1e40af;font-size:13px;"); snTxt->setWordWrap(true);
...
QWidget* DataEntryWizard::buildPage2() {
    ...
    eqTitle->setStyleSheet("color:#1e40af;font-size:13px;font-weight:700;letter-spacing:1px;");
    m_biEquation = new QLabel("σ²(BI) = (σ_BI)²  =  0.0000 kg²");
    m_biEquation->setStyleSheet("color:#1e40af;font-family:'Courier New',monospace;font-size:14px;font-weight:700;");
    ...
    if (auto* le = m_biLabel->lineEdit()) {
        ...
        le->setStyleSheet("background:transparent;border:none;color:#111827;"
                          "font-size:14px;padding:0px 3px;");
    ...
    m_biMethod->setMinimumWidth(320);
    m_biMethod->setStyleSheet(
        "QComboBox{background:white;border:1.5px solid #d1d5db;border-radius:6px;"
        "color:#111827;padding:7px 12px;font-size:14px;}"
        "QComboBox:focus{border:2px solid #1a3a5c;}"
        "QComboBox::drop-down{border-left:1px solid #d1d5db;width:22px;}"
        "QComboBox QAbstractItemView{font-size:13px;min-width:420px;}"
        "QComboBox QAbstractItemView::item{padding:4px 8px;min-height:22px;}");
    ...
    m_biEpsLbl->setStyleSheet("color:#1e40af;font-size:14px;font-family:'Courier New',monospace;");
    ...
    refT->setStyleSheet("color:#0369a1;font-weight:700;font-size:14px;");
    ...
    refD->setStyleSheet("color:#0c4a6e;font-size:13px;"); refD->setWordWrap(true);
    ...
    pnT->setStyleSheet("color:#15803d;font-weight:700;font-size:14px;");
    auto* pnD = new QLabel(
    ...
    pnD->setStyleSheet("color:#166534;font-size:13px;"); pnD->setWordWrap(true);
...
QWidget* DataEntryWizard::buildPage3() {
    ...
    m_inEquation->setStyleSheet("color:#1e40af;font-family:'Courier New',monospace;font-size:14px;");
    ...
    vlimit->setStyleSheet("color:#111827;font-size:13px;");
    ...
    m_inputsTable->setColumnWidth(1, 150);
    m_inputsTable->setColumnWidth(2, 280);
    m_inputsTable->setColumnWidth(3, 220);
    ...
    snT->setStyleSheet("color:#92400e;font-size:13px;"); snT->setWordWrap(true);
    snL->addWidget(snT);
...
QWidget* DataEntryWizard::buildPage4() {
    ...
    m_outEquation->setStyleSheet("color:#1e40af;font-family:'Courier New',monospace;font-size:14px;");
    ...
    m_outputsTable->setColumnWidth(1, 150);
    m_outputsTable->setColumnWidth(2, 280);
    m_outputsTable->setColumnWidth(3, 220);
    ...
    onT->setStyleSheet("color:#166534;font-size:13px;"); onT->setWordWrap(true);
    onL->addWidget(onT);
...
QWidget* DataEntryWizard::buildPage5() {
    ...
    eqTitleLbl->setStyleSheet("color:#1e40af;font-size:13px;font-weight:700;letter-spacing:1px;");
    m_eiEquation = new QLabel("Awaiting EI entry...");
    m_eiEquation->setStyleSheet("color:#1e40af;font-family:'Courier New',monospace;font-size:14px;font-weight:700;");
    ...
    m_eiLabel->setMinimumContentsLength(22);
    m_eiLabel->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
    m_eiLabel->setMinimumWidth(380);
    ...
    m_eiMethod->setMinimumWidth(320);
    ...
    m_eiEpsLbl->setStyleSheet("color:#1e40af;font-size:14px;font-family:'Courier New',monospace;");
    ...
    sqTitle->setStyleSheet("color:#92400e;font-weight:700;font-size:14px;");
    ...
    sqData->setStyleSheet("color:#78350f;font-size:13px;font-family:'Courier New',monospace;");
...
void DataEntryWizard::addTableRow(QTableWidget* tbl, const QString& prefix) {
    ...
    descCombo->setStyleSheet(
        ...
        "padding:5px 8px 5px 10px;color:#111827;font-size:13px;}"
        "QComboBox:focus{border:1.5px solid #1a3a5c;}"
        "QComboBox::drop-down{border-left:1px solid #d1d5db;width:24px;}"
        "QComboBox QAbstractItemView{font-size:13px;min-width:500px;}"
    ...
    qty->setMinimumWidth(120);
    qty->setStyleSheet(
        "QDoubleSpinBox{background:white;border:1px solid #d1d5db;"
        "border-radius:4px;padding:5px 6px;color:#111827;font-size:13px;}"
        "QDoubleSpinBox::up-button,QDoubleSpinBox::down-button{width:18px;}");
    ...
    methodCombo->setStyleSheet(
        "QComboBox{background:white;border:1px solid #d1d5db;border-radius:4px;"
        "padding:5px 8px;color:#111827;font-size:13px;}"
        "QComboBox:focus{border:1.5px solid #1a3a5c;}"
        "QComboBox::drop-down{border-left:1px solid #d1d5db;width:22px;}"
        "QComboBox QAbstractItemView{font-size:13px;min-width:340px;}"
...
MufInput DataEntryWizard::collectInput() const {
    MufInput inp;
...
void DataEntryWizard::loadScenario(const MufInput& inp) {
    // ── Page 1: Facility ────────────────────────────────────────────
    ...
    m_biLabel->setCurrentText(inp.beginningInventory.name);
    m_biQty->setValue(inp.beginningInventory.weight);
    m_biErr->setValue(inp.beginningInventory.uncertainty);
    ...
    for (auto& item : inp.inputs) {
        addTableRow(m_inputsTable, "Input");
        int r = m_inputsTable->rowCount() - 1;
        if (auto* w = qobject_cast<QComboBox*>(m_inputsTable->cellWidget(r, 0)))      w->setCurrentText(item.name);
        if (auto* w = qobject_cast<QDoubleSpinBox*>(m_inputsTable->cellWidget(r, 1))) w->setValue(item.weight);
    ...
    for (auto& item : inp.outputs) {
        addTableRow(m_outputsTable, "Output");
        int r = m_outputsTable->rowCount() - 1;
        if (auto* w = qobject_cast<QComboBox*>(m_outputsTable->cellWidget(r, 0)))      w->setCurrentText(item.name);
        if (auto* w = qobject_cast<QDoubleSpinBox*>(m_outputsTable->cellWidget(r, 1))) w->setValue(item.weight);
    ...
    m_eiLabel->setCurrentText(inp.endingInventory.name);
    m_eiQty->setValue(inp.endingInventory.weight);
    { int idx = m_eiMethod->findData("manual"); if (idx>=0) m_eiMethod->setCurrentIndex(idx); }
    m_eiErr->setValue(inp.endingInventory.uncertainty);
