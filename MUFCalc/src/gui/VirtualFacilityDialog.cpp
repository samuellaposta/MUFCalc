...
void VirtualFacilityDialog::buildFacilities() {
    ...
    // ─────────────────────────────────────────────────────────────────
    {
        ...
        f.primaryKmp = "KMP-B — LEU Product UF6 Cylinders";
        f.primaryKmpRationale =
            "All worked examples for this facility are anchored on KMP-B (LEU product). "
            "The literature shows MUF/σ(MUF) figures vary by which KMP authors choose as "
            "reference; we fix on KMP-B because product cylinders are weighed on certified "
            "calibrated scales with the smallest, best-characterised σ at the plant. "
            "Feed (KMP-A) and tails (KMP-C) appear in the balance but the calculation "
            "is presented from the KMP-B perspective.";
        f.methodsTableHtml =
            "<h3>Measurement Methods &amp; Typical 1σ (UF6 Enrichment Plant)</h3>"
            "<p>Different streams use different instruments. The 1σ values below are <strong>typical</strong> "
            "ranges from IAEA STR-368 for this facility type; the actual σ used in each scenario "
            "is derived from the declared quantity × ε for the chosen method.</p>"
            "<table><tr><th>Stream / KMP</th><th>Method</th><th>Typical 1σ (relative)</th></tr>"
            "<tr><td>UF6 product cylinder mass (KMP-B)</td><td>Calibrated industrial scale, double weighing</td><td>0.05 – 0.25%</td></tr>"
            "<tr><td>UF6 feed cylinder mass (KMP-A)</td><td>Same as KMP-B</td><td>0.20 – 0.50%</td></tr>"
            "<tr><td>UF6 tails cylinder mass (KMP-C)</td><td>Industrial scale (less frequent calibration)</td><td>0.25 – 0.50%</td></tr>"
            "<tr><td>U-235 enrichment assay</td><td>Mass spectrometry / NDA gamma</td><td>0.10 – 0.30%</td></tr>"
            "<tr><td>In-plant cascade hold-up</td><td>NDA + mass-balance estimate</td><td>2 – 5% (much higher)</td></tr>"
            "<tr><td>Samples to IAEA SAL</td><td>Analytical balance + DA</td><td>1 – 3% absolute</td></tr></table>"
            "<div class='tip'><strong>Why one formula does not fit all:</strong> σ is computed differently "
            "per stream because the underlying measurement chain differs. A single uniform percentage applied "
            "to every line item would mis-state σ(MUF) — under-reporting hold-up uncertainty and over-reporting "
            "product uncertainty. Each scenario in this facility uses ε values consistent with this table.</div>"
            "<div class='kmp'><strong>Bias vs uncertainty (sign convention):</strong> the <em>1σ uncertainty</em> "
            "is a standard deviation and is <strong>always positive</strong> — it describes precision/spread. "
            "A <em>bias</em> Δ is a <strong>signed</strong> systematic offset. A measurement may have +0.05 kg "
            "bias (read high) or −0.05 kg bias (read low). Bias is reported alongside σ and is shown in the "
            "NMA Data table as a signed Δ column.</div>";
        ...
        // Scenario 1.1 — Normal Quarter
        {
            ...
            in.inputs  = {
                {"Batch-1: Natural UF6 Feed Receipt (Jan)",       2500.0, 6.25, +1.20},  // calibrated scale runs slightly high
                {"Batch-2: Natural UF6 Feed Receipt (Feb)",       2200.0, 5.50, -0.85},  // re-cal in Feb shifted bias negative
                {"Batch-3: Natural UF6 Feed Receipt (Mar)",       1800.0, 4.50},
            };
            in.outputs = {
                {"LEU Product UF6 3.5% (KMP-B, Jan-Mar)",        2800.0, 7.00, +0.40},  // KMP-B reference, smallest bias
                {"Depleted UF6 Tails 0.3% (KMP-C, Jan-Mar)",     1900.0, 4.75, -1.10},  // tails scale reads low
        ...
        // Scenario 1.2 — Anomalous Quarter (missing tails declaration)
        {
            ...
            in.outputs = {
                {"LEU Product UF6 3.5% (KMP-B)",    2950.0, 7.375, +0.55},  // KMP-B small positive bias
        ...
        // Scenario 1.3 — Critical MUF exceeds 3σ (diversion simulation)
        {
            ...
            in.outputs = {
                ...
                {"LEU Product UF6 3.5% (KMP-B)",  2380.0, 5.95, +0.45},  // KMP-B; underdeclared by ~620 kg → MUF spike
    ...
    // ─────────────────────────────────────────────────────────────────
    {
        ...
        f.primaryKmp = "KMP-2 — MOX Fuel Assembly Output";
        f.primaryKmpRationale =
            "The worked example is anchored on KMP-2 (finished MOX assemblies shipped). "
            "Fresh Pu oxide receipts (KMP-1), scrap (KMP-3) and waste (KMP-4) appear in "
            "the balance but the σ analysis is presented from the KMP-2 perspective because "
            "shipped assemblies are the most accurately measured stream (item-level NDA + "
            "shipper-receiver reconciliation).";
        f.methodsTableHtml =
            "<h3>Measurement Methods &amp; Typical 1σ (MOX Fuel Fabrication)</h3>"
            "<p>Pu accountancy uses a different instrument set than uranium accountancy. STR-368 "
            "values for the typical methods at MOX fabrication are:</p>"
            "<table><tr><th>Stream / KMP</th><th>Method</th><th>Typical 1σ (relative)</th></tr>"
            "<tr><td>MOX assembly Pu content (KMP-2)</td><td>Active/passive neutron coincidence</td><td>0.4 – 0.6%</td></tr>"
            "<tr><td>Fresh Pu oxide receipt (KMP-1)</td><td>Calorimetry + isotopics</td><td>0.3 – 0.5%</td></tr>"
            "<tr><td>Pu scrap return (KMP-3)</td><td>Neutron coincidence + chemical assay</td><td>0.5 – 1.0%</td></tr>"
            "<tr><td>Pu-bearing waste (KMP-4)</td><td>Drum NDA (segmented gamma scan)</td><td>5 – 15%</td></tr>"
            "<tr><td>In-process hold-up (powders, dies, ducts)</td><td>NDA estimate + clean-out</td><td>3 – 10%</td></tr>"
            "<tr><td>Samples to SAL</td><td>Analytical balance</td><td>1 – 3%</td></tr></table>"
            "<div class='tip'><strong>Why one formula does not fit all:</strong> finished assembly Pu (≈0.5%) "
            "is two orders of magnitude better measured than Pu in waste drums (≈10%). σ(MUF) at MOX is "
            "almost always dominated by the waste / hold-up terms, not the assembly term.</div>"
            "<div class='kmp'><strong>Bias vs uncertainty (sign convention):</strong> 1σ is positive by "
            "definition (it is a standard deviation). The signed bias Δ in the NMA Data table reflects "
            "calibration offsets — for example a coincidence counter may consistently read +0.02 kg high "
            "on assembly Pu, while waste drum NDA may read −0.05 kg low. These are reported separately.</div>";
        ...
        // Scenario 2.1 — Normal Month (Beginner)
        {
            ...
            in.outputs = {
                {"MOX fuel assemblies shipped (8 assemblies)", 6.12, 0.031, +0.018},  // KMP-2 (primary); coincidence counter slight high bias
                {"Pu-bearing waste to interim storage",        0.95, 0.048, -0.040},  // drum NDA reads low (typical)
                {"Process scrap to scrap recovery",           0.44, 0.022},
                {"Measurement samples to SAL",                0.01, 0.001},
            };
            in.endingInventory = {"Pu oxide + in-process inventory (PIV Jan 31)", 26.10, 0.129, -0.015};  // EI reads slightly low → MUF negative
            f.scenarios.append(s);
        }
        ...
        // Scenario 2.2 — CUSUM Trend (Intermediate)
        {
            ...
            in.outputs = {
                {"MOX fuel assemblies shipped (10 assemblies)", 6.80, 0.034, +0.022},  // KMP-2 (primary)
                {"Pu waste to storage",                         0.88, 0.044, -0.038},
    ...
    // ─────────────────────────────────────────────────────────────────
    {
        ...
        f.primaryKmp = "KMP-IAT — Input Accountability Tank (Pu nitrate)";
        f.primaryKmpRationale =
            "All worked examples for this facility are anchored on the Input Accountability "
            "Tank (KMP-IAT). The IAT is the single most heavily instrumented and "
            "internationally-agreed reference point at any reprocessing plant; it is also where "
            "shipper-receiver reconciliation is performed against the reactor operator's burn-up "
            "calculation. Other points (product output, waste, hold-up) are included in the "
            "balance but are documented relative to KMP-IAT to avoid the inter-source "
            "inconsistencies the reviewer flagged in the literature.";
        f.methodsTableHtml =
            "<h3>Measurement Methods &amp; Typical 1σ (PUREX Reprocessing)</h3>"
            "<p>Reprocessing has the widest spread of measurement quality of any facility type. "
            "STR-368 typical values:</p>"
            "<table><tr><th>Stream / KMP</th><th>Method</th><th>Typical 1σ (relative)</th></tr>"
            "<tr><td>Input accountability tank (KMP-IAT)</td><td>Calibrated tank gauging + DA isotope-dilution mass spec</td><td>0.5 – 1.0%</td></tr>"
            "<tr><td>Pu product cylinder shipped</td><td>Calibrated balance + chemical assay</td><td>0.3 – 0.5%</td></tr>"
            "<tr><td>Reactor-declared Pu (shipper)</td><td>Burn-up calculation from operating data</td><td>2 – 5%</td></tr>"
            "<tr><td>High-level waste (vitrified)</td><td>Indirect mass-balance, analytical sampling</td><td>10 – 30%</td></tr>"
            "<tr><td>In-process hold-up (solvents, columns)</td><td>Process model + post-clean-out DA</td><td>10 – 30%</td></tr>"
            "<tr><td>Samples to SAL</td><td>Analytical balance + DA</td><td>1 – 3%</td></tr></table>"
            "<div class='tip'><strong>Why one formula does not fit all:</strong> at PUREX, σ for the "
            "input tank (≈0.7%) and σ for hold-up (≈20%) differ by a factor of ~30. Treating them "
            "the same would produce a meaningless σ(MUF). The educational point is that ε must be "
            "selected per stream, per measurement chain.</div>"
            "<div class='kmp'><strong>Bias vs uncertainty (sign convention):</strong> systematic bias "
            "Δ at PUREX is typically signed and material — e.g. shipper-receiver differences come from "
            "biases in the reactor's burn-up code (often + or − a few tenths of a percent). The Δ column "
            "in the NMA Data tab makes these signed offsets visible without inflating the 1σ.</div>";
        ...
        // Scenario 3.1 — Normal
        {
            ...
            in.inputs  = {
                {"Irradiated fuel receipt — Pu content (calc.)", 14.20, 0.426, -0.28},  // KMP-IAT (primary); shipper burn-up reads low
                {"Recycled Pu from previous batch",               0.85, 0.043},
            };
            in.outputs = {
                {"Pu nitrate product shipped to MOX plant",  12.10, 0.061, +0.040},  // calibrated; small positive bias
                {"High-level waste (Pu in vitrified glass)",  0.48, 0.144, -0.060},  // HLW NDA chronically low
                {"Intermediate-level waste",                  0.12, 0.036},
                {"Process holdup decrease (measured)",        0.45, 0.135},  // 30% holdup unc
            };
            in.endingInventory = {"Pu in process + product store (PIV)", 21.40, 0.404, -0.18};  // PIV re-measure higher → MUF goes negative
            f.scenarios.append(s);
        }
        ...
        // Scenario 3.3 — CUSUM + SRD Combined (Advanced)
        {
            ...
            in.inputs  = {
                ...
                {"Irradiated fuel Pu — receiver measurement (KMP input)", 16.40, 0.492, -0.80},  // KMP-IAT (primary); receiver < shipper → SRD
    ...
    // ─────────────────────────────────────────────────────────────────
    {
        ...
        f.primaryKmp = "KMP-Core — Reactor Core Fuel Inventory";
        f.primaryKmpRationale =
            "Worked examples are anchored on the reactor core fuel inventory (KMP-Core). "
            "Because fuel elements are discrete, item-counted, and individually assayed before "
            "loading, KMP-Core is the lowest-uncertainty reference point at the facility. "
            "Fresh-fuel store, spent-fuel store, and target irradiation streams appear in the "
            "balance but are presented from the KMP-Core perspective.";
        f.methodsTableHtml =
            "<h3>Measurement Methods &amp; Typical 1σ (Research Reactor)</h3>"
            "<p>Item-level accountancy gives this facility the smallest σ% of the four reference "
            "facilities, but the per-element absolute σ matters because fuel elements are small.</p>"
            "<table><tr><th>Stream / KMP</th><th>Method</th><th>Typical 1σ (relative)</th></tr>"
            "<tr><td>Fresh HEU fuel element receipt</td><td>Item count + manufacturer's certified U-235 mass</td><td>0.05 – 0.20%</td></tr>"
            "<tr><td>Reactor core inventory (KMP-Core)</td><td>Item identification + reactor records</td><td>0.05 – 0.10%</td></tr>"
            "<tr><td>Spent fuel shipped</td><td>Item count + burn-up correction (operator)</td><td>0.30 – 0.50%</td></tr>"
            "<tr><td>NatU target discs irradiated</td><td>Bulk weighing + analytical assay</td><td>0.50 – 1.0%</td></tr>"
            "<tr><td>HEU samples to SAL (DA)</td><td>Analytical balance</td><td>1 – 3% absolute</td></tr></table>"
            "<div class='tip'><strong>Why one formula does not fit all:</strong> fresh fuel ε is dominated "
            "by manufacturer's certificate (~0.1%), but spent fuel ε is dominated by burn-up uncertainty "
            "(~0.4%). Using a uniform ε would mis-rank the dominant uncertainty source.</div>"
            "<div class='kmp'><strong>Bias vs uncertainty (sign convention):</strong> at a research reactor "
            "the dominant signed bias is the burn-up correction on spent fuel — typically negative (operator "
            "burn-up codes slightly under-predict actinide depletion). The Δ column shows this signed.</div>";
        ...
        // Scenario 4.1 — HEU to LEU conversion (Beginner)
        {
            ...
            in.inputs  = {
                {"Fresh HEU fuel elements received (2 elements)",  1.920, 0.010, +0.003},  // mfg. certificate slight over-statement
            };
            in.outputs = {
                {"Spent HEU fuel elements shipped to storage (3)", 2.855, 0.014, -0.012},  // burn-up code under-predicts depletion (typical)
        ...
        // Scenario 4.2 — Natural Uranium targets (Intermediate)
        {
            ...
            in.outputs = {
                {"Irradiated NatU targets shipped to radiochem lab", 21.60, 0.216, -0.05},
                {"NatU scrap / trim waste from target fabrication",   0.78, 0.039},
            };
            in.endingInventory = {"NatU targets remaining in storage (PIV)", 20.18, 0.200, -0.10};  // MUF flips slightly negative
            f.scenarios.append(s);
        }
...
void VirtualFacilityDialog::buildUI() {
    ...
    t1->setStyleSheet("color:white;font-size:20px;font-weight:900;background:transparent;");
    auto* t2 = new QLabel("4 nuclear facility replicas with integrated NMA training scenarios");
    t2->setStyleSheet("color:#93c5fd;font-size:13px;background:transparent;");
    ...
    hdBadge->setStyleSheet("color:#93c5fd;font-size:13px;background:transparent;");
    ...
    facHdr->setStyleSheet("background:#e2eaf2;color:#1a3a5c;"
                          "font-size:12px;font-weight:700;letter-spacing:2px;padding:10px 16px;"
    ...
    scenHdr->setStyleSheet("background:#e2eaf2;color:#1a3a5c;"
                           ...
                           "padding:10px 16px;border-top:1px solid #d1d8e0;"
                           "border-bottom:1px solid #d1d8e0;");
    ...
    m_scenarioList->setStyleSheet(
        ...
        "font-size:13px;color:#1a3a5c;font-family:'Courier New',monospace;}"
        "QListWidget::item:selected{background:#1a3a5c;color:white;}"
        "QListWidget::item:hover:!selected{background:#dce8f0;color:#0f2438;}");
    ...
    tabs->setStyleSheet(
        ...
        "QTabBar::tab{padding:10px 26px;font-size:14px;font-weight:700;"
    ...
    // Tab 3 – Live NMA Simulation
    {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            m_stepLabels[i]->setStyleSheet(
                ...
                : "background:#e5ecf2;color:#374151;border-radius:6px;"
    ...
    note->setStyleSheet("color:#4a6a8a;font-size:13px;background:transparent;");
    auto* closeBtn = new QPushButton("Close");
    closeBtn->setStyleSheet(Styles::secondaryButtonStyle() +
                             "QPushButton{padding:7px 18px;font-size:13px;}");
...
void VirtualFacilityDialog::updateDetailView() {
    ...
    m_diffBadge->setStyleSheet(diffStyle + "border-radius:14px;padding:5px 14px;"
    ...
    // Primary KMP anchor (single key location for the worked example) ─────
    if (!fac.primaryKmp.isEmpty()) {
        html += "<h2>Primary KMP for Worked Example</h2>";
        html += "<div class='kmp'><strong>" + fac.primaryKmp + "</strong><br>"
              + fac.primaryKmpRationale + "</div>";
    }

    // Per-stream measurement methods & typical 1σ values ──────────────────
    if (!fac.methodsTableHtml.isEmpty()) {
        html += "<h2>Measurement Methods &amp; Uncertainty by Stream</h2>";
        html += fac.methodsTableHtml;
    }
    ...
    html += "<li>Review the pre-filled values — note the <strong>Bias Δ</strong> column on the NMA Data tab. Δ is the <em>signed</em> systematic offset and may be positive (read high) or negative (read low); 1σ remains a non-negative random uncertainty.</li>";
    html += "<li>Note that not every stream uses the same measurement formula — see the Methods table above for the technique applied to each stream.</li>";
    ...
    nhtml += QString("<tr><td>%1</td><td><code>%2</code></td><td><code>%3</code></td><td>%4%</td></tr>")
        .arg(inp.beginningInventory.name)
        .arg(inp.beginningInventory.weight, 0,'f',3)
        .arg(inp.beginningInventory.uncertainty, 0,'f',4)
        .arg(inp.beginningInventory.weight > 0 ? inp.beginningInventory.uncertainty/inp.beginningInventory.weight*100 : 0, 0,'f',2);
    nhtml += "</table>";

    nhtml += "<h2>Material Inputs</h2>";
    nhtml += "<table><tr><th>Description</th><th>Quantity (kg)</th><th>1&#963; (kg)</th><th>Bias &#916; (kg)</th></tr>";
    double totalIn = 0;
    for (auto& it : inp.inputs) {
        QString biasCell;
        if (std::abs(it.bias) < 1e-12) {
            biasCell = "<span style='color:#6b7280;'>0.000</span>";
        } else {
            QString color = it.bias > 0 ? "#b91c1c" : "#15803d";   // red = read high, green = read low
            QString sign  = it.bias > 0 ? "+" : "";
            biasCell = QString("<span style='color:%1;font-weight:600;'>%2%3</span>")
                        .arg(color).arg(sign).arg(it.bias, 0, 'f', 4);
        }
        nhtml += QString("<tr><td>%1</td><td><code>%2</code></td><td><code>%3</code></td><td><code>%4</code></td></tr>")
            .arg(it.name).arg(it.weight,0,'f',3).arg(it.uncertainty,0,'f',4).arg(biasCell);
        totalIn += it.weight;
    }
    nhtml += QString("<tr style='background:#f0f2f5;font-weight:700;'>"
                     "<td>TOTAL INPUTS</td><td><code>%1</code></td><td>—</td><td>—</td></tr>")
        .arg(totalIn,0,'f',3);
    nhtml += "</table>";
    ...
    for (auto& it : inp.outputs) {
        ...
        if (std::abs(it.bias) < 1e-12) {
            ...
            QString color = it.bias > 0 ? "#b91c1c" : "#15803d";
            QString sign  = it.bias > 0 ? "+" : "";
            biasCell = QString("<span style='color:%1;font-weight:600;'>%2%3</span>")
                        .arg(color).arg(sign).arg(it.bias, 0, 'f', 4);
        }
    ...
    nhtml += QString("<tr style='background:#f0f2f5;font-weight:700;'>"
                     "<td>TOTAL OUTPUTS</td><td><code>%1</code></td><td>—</td><td>—</td></tr>")
        .arg(totalOut,0,'f',3);
    nhtml += "</table>";
    nhtml += "<p style='font-size:14px;color:#475569;margin-top:-4px;'>"
             "<strong>Bias &#916; legend:</strong> "
             "<span style='color:#b91c1c;font-weight:600;'>+value</span> = instrument reads high &nbsp;|&nbsp; "
             "<span style='color:#15803d;font-weight:600;'>−value</span> = instrument reads low &nbsp;|&nbsp; "
             "1&#963; is always non-negative (it is a standard deviation).</p>";

    nhtml += "<h2>Ending Inventory (EI)</h2>";
    nhtml += "<table><tr><th>Description</th><th>Quantity (kg)</th><th>1&#963; (kg)</th><th>&#963;%</th></tr>";
    nhtml += QString("<tr><td>%1</td><td><code>%2</code></td><td><code>%3</code></td><td>%4%</td></tr>")
        .arg(inp.endingInventory.name)
        .arg(inp.endingInventory.weight, 0,'f',3)
        .arg(inp.endingInventory.uncertainty, 0,'f',4)
        .arg(inp.endingInventory.weight > 0 ? inp.endingInventory.uncertainty/inp.endingInventory.weight*100 : 0, 0,'f',2);
    nhtml += "</table>";

    // Quick hand calculation preview
    double IB  = inp.beginningInventory.weight + totalIn - totalOut;
    double MUF = IB - inp.endingInventory.weight;
    double varBI  = inp.beginningInventory.uncertainty * inp.beginningInventory.uncertainty;
    double varIn  = 0, varOut = 0;
    for (auto& it : inp.inputs)  varIn  += it.uncertainty * it.uncertainty;
    for (auto& it : inp.outputs) varOut += it.uncertainty * it.uncertainty;
    double varEI  = inp.endingInventory.uncertainty * inp.endingInventory.uncertainty;
...
void VirtualFacilityDialog::onRunNMACalculation() {
    ...
    auto highlight = [](int step, QLabel* lbl[5]) {
        for (int i = 0; i < 5; ++i)
            lbl[i]->setStyleSheet(i == step
                ? "background:#1a3a5c;color:white;border-radius:6px;padding:6px 12px;font-size:13px;font-weight:700;"
                : (i < step
                    ? "background:#5da832;color:white;border-radius:6px;padding:6px 12px;font-size:13px;font-weight:700;"
                    : "background:#d1d5db;color:#111827;border-radius:6px;padding:6px 12px;font-size:13px;font-weight:600;"));
    };

    // STEP 1 — Load data
    highlight(0, m_stepLabels);

    double BI      = inp.beginningInventory.weight;
    double sigBI   = inp.beginningInventory.uncertainty;
    double totalIn = 0, varIn  = 0;
    double totalOut= 0, varOut = 0;
    for (auto& it : inp.inputs)  { totalIn  += it.weight; varIn  += it.uncertainty*it.uncertainty; }
    for (auto& it : inp.outputs) { totalOut += it.weight; varOut += it.uncertainty*it.uncertainty; }
    double EI      = inp.endingInventory.weight;
    double sigEI   = inp.endingInventory.uncertainty;
    ...
    for (auto& it : inp.inputs) {
        ...
        html += QString("<tr><td>&nbsp;&nbsp;&nbsp;+ Input: %1</td><td><code>%2</code></td><td><code>%3</code></td><td><code>%4</code></td></tr>")
                .arg(it.name).arg(it.weight,0,'f',3).arg(it.uncertainty,0,'f',4).arg(it.uncertainty*it.uncertainty,0,'f',4);
    }
    double runOut = 0;
    for (auto& it : inp.outputs) {
        runOut += it.weight;
    ...
    for (auto& it : inp.inputs)  contrib("Input: " + it.name, it.uncertainty*it.uncertainty);
    for (auto& it : inp.outputs) contrib("Output: " + it.name, it.uncertainty*it.uncertainty);
