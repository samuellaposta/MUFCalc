...
// ── Shared HTML CSS — aligned with the Virtual Facility Scenario page ─
static const QString CSS = R"(
<style>
  body  { font-family:'Segoe UI',Ubuntu,Arial,sans-serif;
          font-size:18px; color:#111827; background:white;
          margin:0; padding:0; line-height:1.7; }
  h1    { color:#1a3a5c; font-size:25px; font-weight:900;
          border-bottom:3px solid #1a3a5c; padding-bottom:10px; margin-bottom:18px; }
  h2    { color:#1a3a5c; font-size:22px; font-weight:800;
          margin-top:26px; margin-bottom:10px; }
  h3    { color:#2d5680; font-size:20px; font-weight:700;
          margin-top:20px; margin-bottom:8px; }
  p     { margin:10px 0; }
  code  { background:#f0f2f5; border:1px solid #e5e7eb; border-radius:4px;
          padding:2px 8px; font-family:'Courier New',monospace; font-size:17px;
          color:#1a3a5c; }
  pre   { background:#f0f2f5; border:1px solid #e5e7eb; border-radius:8px;
          padding:14px 18px; font-family:'Courier New',monospace; font-size:17px;
          color:#1a3a5c; margin:12px 0; white-space:pre-wrap; }
  .step { background:white; border:1px solid #e5e7eb;
          border-left:5px solid #1a3a5c; border-radius:8px;
          padding:16px 20px; margin:14px 0; }
  .step-num { color:#1a3a5c; font-size:22px; font-weight:900;
              float:left; margin-right:14px; line-height:1; }
  ...
  .badge { display:inline-block; padding:3px 11px; border-radius:12px;
           font-size:16px; font-weight:700; margin-right:6px; }
  ...
  dt { font-weight:700; color:#1a3a5c; margin-top:14px; font-size:18px; }
...
QTextBrowser* StudentGuideDialog::makeBrowser(const QString& html) {
    auto* b = new QTextBrowser();
    b->setHtml(CSS + html);
    b->setOpenExternalLinks(true);
    b->setStyleSheet("QTextBrowser{background:white;border:none;padding:22px;font-size:14px;}");
    return b;
}
...
//  CONSTRUCTOR
// ══════════════════════════════════════════════════════════════════════
StudentGuideDialog::StudentGuideDialog(QWidget* parent) : QDialog(parent) {
...
void StudentGuideDialog::buildUI() {
    ...
    t1->setStyleSheet("color:white;font-size:18px;font-weight:900;");
    auto* t2 = new QLabel("MUF Calculator — IAEA Nuclear Material Accounting System");
    t2->setStyleSheet("color:#a5c8e8;font-size:13px;");
    ...
    badge->setStyleSheet("color:#a5c8e8;font-size:13px;");
    hl->addWidget(badge);
    root->addWidget(hdr);
    ...
    note->setStyleSheet("color:#111827;font-size:13px;");
...
// ══════════════════════════════════════════════════════════════════════
void StudentGuideDialog::buildLessons() {
    m_lessons.clear();
...
QWidget* StudentGuideDialog::buildInteractiveTutorial() {
    buildLessons();
    ...
    progTitle->setStyleSheet("color:#1a3a5c;font-weight:700;font-size:14px;");
    m_progressLbl = new QLabel("0 / 8 lessons completed");
    m_progressLbl->setStyleSheet("color:#111827;font-size:14px;");
    ...
    listHdr->setStyleSheet("background:#1a3a5c;color:#a5c8e8;font-size:13px;font-weight:700;"
                           "letter-spacing:1.5px;padding:10px 16px;");
    leftL->addWidget(listHdr);

    m_lessonList = new QListWidget();
    m_lessonList->setStyleSheet(
        "QListWidget{background:#f4f5f7;border:none;font-size:14px;}"
    ...
    connect(m_lessonList, &QListWidget::currentRowChanged, this, &StudentGuideDialog::showLesson);
...
void StudentGuideDialog::showLesson(int idx) {
...
void StudentGuideDialog::markComplete(int idx) {
    if (idx < 0 || idx >= m_totalLessons) return;
    if (!m_completed[idx]) {
...
// ═══════════════════════════════════════════════════════════════════════
QWidget* StudentGuideDialog::buildHowToUse() {
    auto* page   = new QWidget();
    ...
    ht->setStyleSheet("color:white;font-size:32px;font-weight:900;");
    auto* hs = new QLabel("A step-by-step guide to performing IAEA Nuclear Material Accounting");
    hs->setStyleSheet("color:#a5c8e8;font-size:15px;");
    ...
    stepsTitle->setStyleSheet("color:#1a3a5c;font-size:16px;font-weight:900;"
    ...
    for (auto& s : steps) {
        ...
        badge->setStyleSheet("background:" + s.color + ";color:white;font-size:30px;font-weight:900;"
        ...
        title->setStyleSheet("color:#1a3a5c;font-size:16px;font-weight:800;");
        auto* desc = new QLabel(s.desc);
        desc->setStyleSheet("color:#374151;font-size:14px;line-height:1.55;");
    ...
    qaTitle->setStyleSheet("color:#1a3a5c;font-size:16px;font-weight:900;"
                           "border-bottom:3px solid #1a3a5c;padding-bottom:10px;");
    ...
    auto makeQACard = [](const QString& icon, const QString& title, const QString& desc,
        ...
        ico->setStyleSheet("font-size:38px;");
...
//  TAB 2 — Quick Start
// ═══════════════════════════════════════════════════════════════════════
QWidget* StudentGuideDialog::buildQuickStart() {
...
//  TAB 3 — IAEA Theory
// ═══════════════════════════════════════════════════════════════════════
QWidget* StudentGuideDialog::buildTheory() {
    return makeBrowser(R"(
...
// ═══════════════════════════════════════════════════════════════════════
QWidget* StudentGuideDialog::buildDiagnosticsGuide() {
    return makeBrowser(R"(
...
//  TAB 5 — Glossary
// ═══════════════════════════════════════════════════════════════════════
QWidget* StudentGuideDialog::buildGlossary() {
    return makeBrowser(R"(
...
//  TAB 6 — FAQ
// ═══════════════════════════════════════════════════════════════════════
QWidget* StudentGuideDialog::buildFAQ() {
    return makeBrowser(R"(
...
// ─────────────────────────────────────────────────────────────────
void StudentGuideDialog::paintEvent(QPaintEvent* e)
{
