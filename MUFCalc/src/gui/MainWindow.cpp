...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    setWindowTitle(
        "MUFCalc v2.0  –  Nuclear Material Accountancy Management System  |  TPU");
...
void MainWindow::buildMenuBar() {
    ...
    // ── Reference Materials (was "Help" in v1) ─────────────────────────
    auto* mRef = menuBar()->addMenu("&Reference Materials");
    mRef->addAction("Student User Guide",          this, &MainWindow::onUserGuide);
    mRef->addSeparator();
    mRef->addAction("Process Flowchart",           this, &MainWindow::onShowFlowchart);
    mRef->addAction("IAEA Equations Diagram",      this, &MainWindow::onShowEquations);
    mRef->addAction("IAEA Reference Equations",    this, &MainWindow::onIAEARef);
    mRef->addAction("Diversion Detection Guide",   this, &MainWindow::onDiversionGuide);
    mRef->addSeparator();
    mRef->addAction("Send Feedback (GitHub)",      this, &MainWindow::onSendFeedback);
    mRef->addAction("&About MUFCalc",              this, &MainWindow::onAbout);
}
...
void MainWindow::buildUI() {
    ...
    sysNameLbl->setStyleSheet(
        "color:#1a3a5c;font-size:19px;font-weight:900;letter-spacing:1px;"
        "background:transparent;");
    auto* sysSubLbl = new QLabel("NUCLEAR MATERIAL ACCOUNTING");
    sysSubLbl->setAlignment(Qt::AlignCenter);
    sysSubLbl->setStyleSheet(
        "color:#4a6a8a;font-size:13px;font-weight:700;letter-spacing:2px;"
        "background:transparent;");
    auto* verLbl = new QLabel("IAEA SAFEGUARDS  v2.0");
    verLbl->setAlignment(Qt::AlignCenter);
    verLbl->setStyleSheet(
        "color:#7a9ab8;font-size:13px;letter-spacing:1.5px;"
    ...
    btnNew->setStyleSheet(Styles::primaryButtonStyle() +
        "QPushButton{text-align:left;padding:13px 16px;font-size:16px;}");
    ...
    btnEx->setStyleSheet(Styles::greenButtonStyle() +
        "QPushButton{text-align:left;padding:11px 16px;font-size:16px;}");
    ...
    toolSepLbl->setStyleSheet(
        "background:#e2eaf2;color:#4a6a8a;font-size:13px;font-weight:700;"
    ...
    auto mkSideBtn = [](const QString& icon, const QString& label) -> QPushButton* {
        ...
        b->setStyleSheet(
            ...
            "text-align:left;padding:11px 20px;font-size:16px;border-radius:0;}"
    ...
    m_historyList->setStyleSheet(
        ...
        "QListWidget::item{padding:10px 16px;border-bottom:1px solid #e8edf2;"
        "color:#1a3a5c;font-size:14px;font-family:'Courier New',monospace;}"
    ...
    dbChip->setStyleSheet(
        "color:#15803d;font-size:13px;font-weight:700;background:transparent;");
    auto* tpuChip = new QLabel("TPU / NSE");
    tpuChip->setStyleSheet(
        "color:#4a6a8a;font-size:13px;background:transparent;");
    ...
    splitter->setSizes({270, 860});
    setCentralWidget(splitter);

    m_statusLbl = new QLabel();
    m_statusLbl->setStyleSheet(
        "font-size:14px;padding:0 8px;font-family:'Courier New',monospace;");
...
QWidget* MainWindow::buildWelcomePage() {
    ...
    bannerL->setContentsMargins(32,14,32,12); bannerL->setSpacing(2);
    auto* bannerTitle = new QLabel("Nuclear Material Accountancy Management System");
    bannerTitle->setStyleSheet(
        "color:white;font-size:24px;font-weight:900;background:transparent;");
    auto* bannerSub = new QLabel(
        "MUFCalc v2.0  ·  IAEA INFCIRC/153 Compliant  ·  "
        "Tomsk Polytechnic University  ·  School of Nuclear Science & Engineering");
    bannerSub->setStyleSheet(
        "color:#c8dff0;font-size:16px;background:transparent;");
    ...
    layout->setContentsMargins(36,26,36,26); layout->setSpacing(24);
    ...
    aoHdr->setStyleSheet(
        "color:#1a3a5c;font-size:16px;font-weight:700;letter-spacing:2px;"
        "background:transparent;");
    ...
    tagline->setStyleSheet(
        "color:#374151;font-size:17px;font-style:italic;"
        "padding:4px 0 10px 0;background:transparent;");
    ...
    for (const auto& a : AIMS) {
        ...
        rowL->setContentsMargins(0,3,0,3); rowL->setSpacing(12);
        auto* numLbl = new QLabel(a.num);
        numLbl->setFixedWidth(32);
        numLbl->setAlignment(Qt::AlignCenter);
        numLbl->setStyleSheet(
            "background:#1a3a5c;color:white;font-size:14px;"
            "font-weight:700;border-radius:2px;padding:4px 0;");
        auto* txt = new QLabel(a.text);
        txt->setWordWrap(true);
        txt->setStyleSheet("color:#1a1a2e;font-size:17px;background:transparent;line-height:1.5;");
    ...
    refHdr->setStyleSheet(
        "color:#4a6a8a;font-size:14px;font-weight:700;"
                        ...
                        const QString& desc, const QString& hint) -> QPushButton*
    {
        ...
        b->setStyleSheet(
            ...
            "font-weight:normal;text-align:left;min-height:96px;}"
    ...
    auto* btnGuide2 = mkNavTile("[?]", "STUDENT GUIDE",
        ...
        "Reference Materials › Student User Guide");
    ...
    auto mkChip = [](const QString& t, const QString& bg,
        ...
        c->setStyleSheet(QString("background:%1;color:%2;border:1px solid %3;"
            "border-radius:4px;padding:5px 11px;font-size:14px;font-weight:700;")
            .arg(bg,fg,bd));
        return c;
    };
    ...
    for (auto& eq : QVector<EQ>{
        ...
        cl->setContentsMargins(14,12,14,14); cl->setSpacing(5);
        auto* t = new QLabel(eq.title);
        t->setStyleSheet(
            QString("color:%1;font-size:17px;font-weight:700;background:transparent;")
            .arg(eq.color));
...
QWidget* MainWindow::buildDashboard() {
    ...
    topTitle->setStyleSheet(
        "color:#1a3a5c;font-size:13px;font-weight:700;letter-spacing:1.5px;"
        "background:transparent;");
    ...
    mufTop->setStyleSheet(
        "color:#4a6a8a;font-size:13px;letter-spacing:1.5px;background:transparent;");
    ...
    auto mkMini = [](QLabel*& ref, const QString& lbl) {
        ...
        ll->setStyleSheet(
            "color:#4a6a8a;font-size:11px;letter-spacing:1px;"
...
void MainWindow::onCalculationDone(const MufInput& input, const MufResult& result) {
    if (!result.success) {
...
void MainWindow::refreshDashboard(const MufInput& input, const MufResult& result) {
    ...
    m_dashStatus->setStyleSheet(
        QString("color:%1;font-size:14px;font-weight:700;background:transparent;").arg(col));
...
// ── Process Flowchart — extract embedded HTML to temp dir, open in browser ──
//    Hardened launcher: ensures the temp directory exists, surfaces the real
//    failure reason if anything goes wrong, and offers a copy-the-path
//    dialog so the user can open the file manually if browser launch fails.
//    Falls back to the static-PNG dialog only if even the embedded resource
//    is missing.
void MainWindow::onShowFlowchart() {
    QString failureReason;

    // 1. Pick a writable directory the user's browser can actually read from.
    //    We deliberately avoid QStandardPaths::TempLocation (/tmp on Linux,
    //    %TEMP% on Windows) because on modern Ubuntu the default Firefox is
    //    a Snap package, and Snap-confined apps cannot read /tmp. The Cache
    //    location (~/.cache/MUFCalc on Linux, %LOCALAPPDATA%\MUFCalc\cache
    //    on Windows) sits inside the user's home where every browser, snap
    //    or otherwise, has access.
    QString cacheDir = QStandardPaths::writableLocation(QStandardPaths::CacheLocation);
    if (cacheDir.isEmpty()) {
        // Last-resort fallback if the platform reports no cache dir at all.
        cacheDir = QDir::homePath() + QStringLiteral("/.MUFCalc");
    }
    if (!QDir().mkpath(cacheDir)) {
        failureReason = "Could not create cache directory: " + cacheDir;
    }

    // 2. Read the embedded HTML and write it to disk.
    const QString tmpPath = QDir::toNativeSeparators(cacheDir + "/MUFCalc_Flowchart.html");
    bool extracted = false;
    if (failureReason.isEmpty()) {
        QFile resHtml(":/MUFCalc_Flowchart.html");
        if (!resHtml.open(QIODevice::ReadOnly)) {
            failureReason = "Embedded resource :/MUFCalc_Flowchart.html could not be opened. "
                            "Build may have skipped the .qrc registration.";
        } else {
            QFile out(tmpPath);
            if (!out.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
                failureReason = "Could not write to: " + tmpPath +
                                " — " + out.errorString();
            } else {
                out.write(resHtml.readAll());
                out.close();
                extracted = true;
            }
            resHtml.close();
    ...
    // 3. Try the browser launch. QUrl::fromLocalFile handles Windows backslashes
    //    correctly (converts to forward slashes in the file:// URL).
    if (extracted) {
        const QUrl url = QUrl::fromLocalFile(tmpPath);
        if (QDesktopServices::openUrl(url))
            return;   // success — interactive flowchart now open in browser

        // 3b. Fallback: Qt's openUrl is unreliable on Windows when the temp path
        //     contains non-ASCII characters or when the .html file association
        //     points at an unusual handler. Try the OS-native shell command
        //     directly. This is what works in practice for almost all users.
        bool launched = false;
#if defined(Q_OS_WIN)
        // The empty "" after "start" is the window-title placeholder, required
        // when the path is quoted by Windows' command processor.
        launched = QProcess::startDetached(
            "cmd.exe", QStringList{"/c", "start", "", tmpPath});
#elif defined(Q_OS_MACOS)
        launched = QProcess::startDetached("open", QStringList{tmpPath});
#else
        launched = QProcess::startDetached("xdg-open", QStringList{tmpPath});
#endif
        if (launched) return;

        failureReason = "Browser launch failed for URL: " + url.toString() +
                        ". Both QDesktopServices and the OS-native shell "
                        "command refused to open the file. Your system may "
                        "have no default browser association, or the file "
                        "path contains characters the browser rejects.";
    }

    // 4. Browser launch failed but the HTML file may still be on disk.
    //    Offer a dialog with the path so the user can open it manually.
    if (extracted) {
        QMessageBox box(this);
        box.setIcon(QMessageBox::Warning);
        box.setWindowTitle(QStringLiteral("Could not open interactive flowchart"));
        box.setText(QStringLiteral(
            "MUFCalc could not launch your default browser to display the "
            "interactive flowchart.\n\n"
            "The HTML file has been written to:\n\n%1\n\n"
            "You can open it manually by copying that path into a browser "
            "address bar, or by double-clicking it from a file manager.").arg(tmpPath));
        box.setDetailedText(failureReason);
        QPushButton* copyBtn  = box.addButton(QStringLiteral("Copy path"),       QMessageBox::ActionRole);
        QPushButton* staticBtn= box.addButton(QStringLiteral("Show static PNG"), QMessageBox::ActionRole);
        QPushButton* closeBtn = box.addButton(QMessageBox::Close);
        box.setDefaultButton(copyBtn);
        box.exec();
        if (box.clickedButton() == copyBtn) {
            QApplication::clipboard()->setText(tmpPath);
            QMessageBox::information(this, QStringLiteral("Copied"),
                QStringLiteral("Flowchart path copied to clipboard."));
            return;
        }
        if (box.clickedButton() == closeBtn) return;
        // staticBtn falls through to the PNG fallback below.
        Q_UNUSED(staticBtn);
    }

    // 5. Static-PNG fallback (resource extraction itself failed, OR user
    //    explicitly clicked "Show static PNG" above).
    auto* dlg = new QDialog(this);
    dlg->setAttribute(Qt::WA_DeleteOnClose);
    dlg->setWindowTitle("Process Workflow Flowchart (static fallback)");
    ...
    t->setStyleSheet("color:#1a3a5c;font-size:14px;font-weight:900;margin-bottom:8px;");
    lay->addWidget(t);
    if (!failureReason.isEmpty()) {
        auto* note = new QLabel(
            "Interactive HTML flowchart unavailable: " + failureReason +
            "\nShowing static PNG instead.");
        note->setStyleSheet("color:#a16207;font-size:12px;margin-bottom:6px;");
    ...
    if (!fc.isNull()) {
        ...
        auto* missing = new QLabel(
            "The static flowchart image is also unavailable. "
            "This indicates the build did not bundle resources/resources.qrc — "
            "see Chapter 9 (Troubleshooting) of the user manual.");
        missing->setStyleSheet("color:#b91c1c;font-size:13px;");
...
void MainWindow::onExportReport() {
    ...
    const MufInput*  inp = (sel>=0 && sel<m_history.size()) ?
...
void MainWindow::onUserGuide()      { StudentGuideDialog dlg(this);  dlg.exec(); }

// ── Send Feedback — opens the FeedbackDialog (API submission + browser fallback)
void MainWindow::onSendFeedback() {
...
void MainWindow::onVirtualFacility() {
    ...
    connect(dlg, &VirtualFacilityDialog::loadScenario, this,
            [this](const MufInput& preloaded) {
        auto* wiz = new DataEntryWizard(this);
        wiz->loadScenario(preloaded);
        connect(wiz, &DataEntryWizard::calculateRequested, this,
                [this, wiz](const MufInput& input) {
